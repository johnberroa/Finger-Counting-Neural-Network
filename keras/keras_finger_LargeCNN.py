...
model = Sequential()
model.add(Conv2D(16, (3, 3), input_shape=input_dims, padding='same', activation='relu'))
...
np.save('tL-acc.npy', hist.history['acc'])
np.save('tL-loss.npy', hist.history['loss'])
np.save('vL-acc.npy', hist.history['val_acc'])
np.save('vL-loss.npy', hist.history['val_loss'])
